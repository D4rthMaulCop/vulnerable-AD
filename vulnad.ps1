#Base Lists 
$Global:HumanFirstNames = @('Jim', 'Pam', 'Michael', 'Dwight', 'Andy', 'Erin', 'Phyllis', 'Stanley', 'Darryl', 'Creed', 'Meredith', 'Ryan', 'Kelly', 'Toby', 'Oscar', 'Kevin', 'Angela', 'David');
$Global:HumanLastNames = @('Halpert', 'Beesly', 'Scott', 'Schrute', 'Bernard', 'Hannon', 'Vance', 'Hudson', 'Philbin', 'Bratton', 'Palmer', 'Howard', 'Kapoor', 'Flenderson', 'Martinez', 'Malone', 'Martin', 'Wallace');
...
function ShowBanner {
    Write-Host "Dunder Mifflin Vulnerable AD Setup Script!"                        
}
...
function VulnAD-AddADUser {
    ...
    for ($i=1; $i -le $limit; $i=$i+1 ) {
        $firstname = ($Global:HumanFirstNames[$i]
    );
        $lastname = ($Global:HumanLastNames[$i]
    );
